...
class SoftwareService {
  ...
  async getSoftware(handle, token) {
    console.log("Entering devices service with handle ", handle, "and token ", token);
    let res = await this.fetch(
           `http://local.topcoder-dev.com/v3/members/${handle}/traits?traitIds=software`,{
             headers: {
         ...
         console.log("Response software", res);
         if (!res.ok) throw new Error(res.statusText);
             if (res.status !== 200) throw new Error(res.content);
             return res.json();
  }
  async updateSoftware(software, handle) {
    console.log("Updated software/software services: ", software);
    const res = await this.private.api.putJsonLocal(`http://local.topcoder-dev.com/v3/members/${handle}/traits`, { param: [software] });
...
export function getService(tokenV3) {
  if (!lastInstance || tokenV3 !== lastInstance.private.tokenV3) {
    lastInstance = new SoftwareService(tokenV3);
  }
  return lastInstance;
}
